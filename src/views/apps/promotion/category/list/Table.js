...
const EmployeesList = () => {
  ...
  async function handleGetListEmployee(value) {
    const result = await promotionCategoryService.getListEmployee(params);
  ...
  const onSubmit = (data) => {
    ...
    setParams((prev) => {
      return {
        ...
        category:{
  name: data?.categoryName?.trim() || null,
  ...
  return (
    <Fragment>
      <ExtensionsHeader title="Khuyến mại loại sản phẩm" />
        ...
        <CardBody className="px-3 py-1">
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Row>
              ...
              <Col md="3" className="mb-2">
                <Label for="search-invoice">Tên thể loại:</Label>
                <Controller
                  name="categoryName"
                  control={control}
                  render={({ field }) => <Input {...field} />}
